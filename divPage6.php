...
?>
 ...
 <?php include("cr.php"); 
$q = mysqli_query($dbcon, "SELECT * FROM leads WHERE sold='0' ORDER BY RAND()")or die(mysqli_error()); while(
	$row = mysqli_fetch_assoc($q)){ 
	$countryfullname = $row['country']; 
	$code = array_search("$countryfullname", 
			     $countrycodes
			    ); 
	$countrycode = strtolower(
		$code
	); 
	$qer = mysqli_query($dbcon, "SELECT * FROM resseller WHERE username='".$row['resseller']."'")or die(mysql_error()); while(
		$rpw = mysqli_fetch_assoc($qer)) 
    ...
    <td>
    <span id='leads_seller'.$row['id'].'" title="seller" type="button"><a href="#" class="btn btn-primary btn-xs">".htmlspecialchars($SellerNick)."</td>
